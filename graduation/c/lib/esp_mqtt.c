...
int join_mqtt(comport_t *comport)
{
    ...
    if( send_atcmd(comport, "AT+TCDEVINFOSET=1,\"UG22BB2M2W\",\"bedroom\",\"0ki4H0K7lujKqi5ZRJx4+A==\"\r\n", 500, AT_EXPSTR, AT_ERRSTR,r_buf, sizeof(r_buf)) <= 0 )
    {
...
int mqtt_sub(comport_t *comport)
{
    ...
    if( send_atcmd(comport, "AT+TCMQTTSUB=\"$thing/down/property/UG22BB2M2W/bedroom\",0\r\n", 2000, AT_EXPSTR, AT_ERRSTR,r_buf, sizeof(r_buf)) <= 0 )
    {
...
int mqtt_pub(comport_t *comport, float *temp, int state, int lightness)
{
    ...
    char s_buf[512];
     ...
     snprintf( s_buf, sizeof(s_buf),"AT+TCMQTTPUB=\"$thing/up/property/UG22BB2M2W/bedroom\",0,\"{\\\"method\\\": \\\"report\\\"\\,\\\"clientToken\\\": \\\"123\\\"\\,\\\"timestamp\\\": 1212121221\\,\\\"params\\\": {\\\"Temperature\\\": %.2f\\,\\\"light\\\": %d\\,\\\"lightness\\\":%d}}\"\r\n\0",*temp, state,lightness );
...
int disconn_mqtt(comport_t *comport)
{
    ...
    //AT+TCMQTTUNSUP 取消订阅
    if( send_atcmd(comport, "AT+TCMQTTUNSUB=\"$thing/down/property/UG22BB2M2W/bedroom\"\r\n", 500, AT_EXPSTR, AT_ERRSTR,r_buf, sizeof(r_buf)) <= 0 )
    {
