...
{  	
	uint8_t  i,j;			
	uint8_t  width=ex-sx+1; 		//get the width of the fill
	uint8_t  height=ey-sy+1;		//get the height of the fill
	for(i=0;i<height;i++)
	{
		for(j=0;j<width;j++)
		{
...
void GUI_DrawLine(uint8_t       x1, uint8_t  y1, uint8_t x2, uint8_t y2,uint8_t color)
{
	...
	delta_x=x2-x1; //Computational coordinate increment
	delta_y=y2-y1; 
	uRow=x1; 
	uCol=y1; 
	...
	if( delta_x>delta_y)distance=delta_x; //Selecting basic incremental axes
	else distance=delta_y; 
	for(t=0;t<=distance+1;t++ )//Line output
	{ 
...
/*****************************************************************************
 * @name       :void GUI_ShowString(uint8_t x,uint8_t y,char *chr,uint8_t Char_Size,uint8_t mode)
 * @date       :2018-08-27 
...
/*****************************************************************************
 * @name       :static uint32_t mypow(uint8_t m,uint8_t n)
 * @date       :2018-08-27 
...
/*****************************************************************************
 * @name       :void GUI_ShowCHinese(uint8_t x,uint8_t y,uint8_t hsize, char *str,uint8_t mode)
 * @date       :2018-08-27 
...
{ 
	while(*str!='\0')
	{
		if(hsize == 16)
		{
			GUI_ShowFont16(x,y,(uint8_t*)str,mode);
		}
		else if(hsize == 24)
		{
			GUI_ShowFont24(x,y,(uint8_t*)str,mode);
		}
		else if(hsize == 32)
		{
			GUI_ShowFont32(x,y,(uint8_t*)str,mode);
		}
		else
		{
			return;
...
/*****************************************************************************
 * @name       :void GUI_DrawBMP(uint8_t x,uint8_t y,uint8_t width, uint8_t height, const uint8_t BMP[], uint8_t mode)
 * @date       :2018-08-27 
